  ...
  return (
    <div className="vehicle-list-content">
      ...
      {!loading && !error && items.length === 0 && (
        <EmptyState title={MESSAGES.EMPTY_RESULTS} description={emptyDescription} />
      )}

      {!loading && !error && items.length > 0 && <div className="reservations-list">{items.map(renderItem)}</div>}
...
export default PeopleResultsPanel;
