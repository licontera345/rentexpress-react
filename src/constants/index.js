/**
 * Punto único de constantes de la aplicación.
 * Uso: import { ROUTES, USER_ROLES, PAGINATION } from './constants';
 */
...
// ─── Estados (IDs/nombres según API; no normalizar) ─────────────────────
...
// ─── Imágenes, formularios y negocio ─────────────────────────────────────
...
export function getVehicleFilterDefaults(options = {}) {
  const { includeIdentifiers = false, includeStatus = false, includeActiveStatus = false } = options;
...
// ─── Sedes (helpers de etiquetas) ─────────────────────────────────────────
function resolveHeadquartersName(hq) {
  return hq?.name ?? '';
}

function resolveHeadquartersAddress(hq) {
  const address = hq?.addresses?.[0];
  const street = address?.street;
  const number = address?.number;
  const cityName = address?.cityName ?? hq?.city?.cityName;
  const provinceName = address?.provinceName ?? hq?.province?.provinceName;
  const streetLine = [street, number].filter(Boolean).join(' ');
  const locationLine = [cityName, provinceName].filter(Boolean).join(', ');
  return [streetLine, locationLine].filter(Boolean).join(', ');
}

export function getHeadquartersOptionLabel(headquarters) {
...
export function getHeadquartersNameLabel(headquarters) {
  return resolveHeadquartersName(headquarters) || '';
}

export function getHeadquartersAddressLabel(headquarters) {
  return resolveHeadquartersAddress(headquarters) || '';
}

export function getHeadquartersCityName(headquarters) {
...
// ─── Mensajes (i18n) ─────────────────────────────────────────────────────
/** Crea el objeto MESSAGES que delega en t(key). Usar cuando i18n esté listo: const MESSAGES = createMessages(t); */
export function createMessages(t) {
  return new Proxy(
    {},
    {
      get(target, prop) {
