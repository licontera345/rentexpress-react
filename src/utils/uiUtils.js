...
/** Resuelve la dirección principal del usuario (contrato OpenAPI: address es array). */
export const resolveAddress = (currentUser) => {
  if (!currentUser) return null;
  return Array.isArray(currentUser.address) ? (currentUser.address[0] ?? null) : null;
};

/** Identificador de entidad autenticada: userId (cliente) o employeeId (empleado). */
